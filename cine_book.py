...
class User:
    ...
    def login(self, username: str, password: str) -> Optional[Dict]:
        ...
        return None
    def book_ticket(self, user_id: int, movie_id: int, theatre_id: int, 
                   seats: int, show_time: str) -> bool:
        try:
            conn = self.db.get_connection()
            cursor = conn.cursor()
            # Get ticket price
                ...
                # Handle duration input - allow both formats
                    ...
                    try:
                        ...
                        if price_input.lower()=='back':
                            break
                        ticket_price = float(input(price_input))
    ...
    def book_with_seat_selection(self, theatre, movie, show_time, seat_info):
        ...
        while True:
            ...
            if confirm.lower() == 'back':
                ...
                if self.user.book_specific_seats(
                    self.current_user['id'], movie['id'], theatre['id'], 
                    show_time, selected_seats
                ):
                    print("Tickets booked successfully!")
                    points_earned = int(total_cost / 10)
