...
class User:
    ...
    def get_seat_arrangement(self, theatre_id: int, movie_id: int, show_time: str) -> Dict:
        ...
        # Create seat map with numeric rows
        seat_map = {}
        for row in range(1, rows + 1):
            row_label=f"Row{row}" #Row1,Row2,Row3,etc.
            seat_map[row_label]={}
            for seat in range(1,seats_per_row+1):
                if (row-1)*seats_per_row+seat<=total_seats:
                    seat_map[row_label][seat]='Available'
        # Mark booked seats
    ...
    def book_specific_seats_with_discount(self, user_id: int, movie_id: int, theatre_id: int, 
                        show_time: str, selected_seats: List[tuple], points_to_redeem: int=0)->tuple: 
        """Book specific seats"""
        try:
            conn = self.db.get_connection()
            cursor = conn.cursor()
            ...
            subtotal=ticket_price*len(selected_seats)
            # Calculate discount
            ...
            cursor.execute(
                ...
                (user_id, movie_id, theatre_id, len(selected_seats), show_time, final_amount, points_earned)
            )
            ...
            new_points=user_points - points_to_redeem + points_earned
            cursor.execute(
            ...
            conn.commit()
            conn.close()
            return True, {"discount":discount, "final_amount": final_amount, "points_earned": points_earned}
...
class CinePredicta:
    ...
    def view_seat_arrangement_interface(self):
        while True:
            ...
            try:
                ...
                for row in sorted(seat_map.keys(),key=lambda x:int(x.replace('Row',''))):
                    row_display = f"{row}: "
    ...
    def book_with_seat_selection(self, theatre, movie, show_time, seat_info):
        ...
        print("\nSelect your seats:")
        print("- Enter one seat at a time (format: Row1-1, Row2-5, etc.)")
        print("- Or enter multiple seats separated by commas (Row1-1, Row2-2,Row3-3)")
        print("- Type 'done' when finished, 'clear' to clear selection, or 'back' to return")
        print(f"DEBUG IMMEDIATELY AFTER SEAT SELECTION: selected_seats = {selected_seats}")
        if selected_seats:
            print(f"DEBUG: First seat type = {type(selected_seats[0])}")
            print(f"DEBUG: First seat value = {selected_seats[0]}")

        while True:
            if selected_seats:
                selected_display = ", ".join([f"{row}-{seat_num}" for row, seat_num in selected_seats])
            ...
            for seat in seat_list:
                ...
                if 'row' in seat.lower() and '-' in seat:
                    parts=seat.split('-')
                    if len(parts)==2: 
                        row=parts[0].title()
                        try:
                            ...
                            if row in seat_map and seat_num in seat_map[row]:
                                ...
                                elif seat_map[row][seat_num]=='Selected':
                                    print(f"Seat {row}-{seat_num} is already in your selection!")
                                else:
                                    print(f"Seat {row}-{seat_num} is not available!")
                            else:
                                print(f"Invalid seat {row}-{seat_num}!")
                        except ValueError:
                            print(f"Invalid seat format: {seat}! Use format like Row1-1, Row2-5")
                    else:
                        print(f"Invalid seat format: {seat}! Use format like Row1-1, Row2-5")
                else:print(f"Invalid seat format: '{seat}'! Use format like Row1-1, Row2-5")  
                print(f"\nDEBUG AFTER SEAT SELECTION:")
                print(f"selected_seats = {selected_seats}")
                print(f"type of selected_seats = {type(selected_seats)}")
                if selected_seats:
                    print(f"type of first element = {type(selected_seats[0])}")
                    print(f"first element value = {selected_seats[0]}")
                    if isinstance(selected_seats[0], tuple):
                        print(f"tuple contents: row='{selected_seats[0][0]}', seat_num={selected_seats[0][1]}")
                print("=" * 50)      
        # Confirm booking
        while True:
            current_points = self.user.get_loyalty_points(self.current_user['id'])
            subtotal = movie['ticket_price'] * len(selected_seats)
            # Debug the variables before booking summary
            print(f"DEBUG: current_points = {current_points}")
            print(f"DEBUG: subtotal = {subtotal}")
            print(f"DEBUG: selected_seats = {selected_seats}")
    
            # Add the missing selected_display definition here
            selected_display = ", ".join([f"{row}-{seat_num}" for row, seat_num in selected_seats])
            print(f"DEBUG: selected_display = {selected_display}")
            print(f"\nBooking Summary:")
            ...
            print(f"Your Loyalty Points: {current_points}")
            # Ask about point redempion
            points_to_redeem=0
            if current_points >= 100:
                max_discount_points = min(current_points,int(subtotal/10)*100)
                max_discount = (max_discount_points//100)*10
                print(f"\nLoyalty Points Redemption: ")
                print(f"You can redeem up to {max_discount_points} points for ${max_discount} discount")
                print("(100 points = $10 discount)")
                redeem_choice=input("Do you want to redeem loyalty points? (y/n): ")
                if redeem_choice.lower() == 'y':
                    while True:
                        try:
                            points_input = input(f"Enter points to redeem (multiples of 100, max {max_discount_points}):")
                            points_to_redeem=int(points_input)
                            if points_to_redeem % 100 != 0:
                                print("Points must be in multiples of 100!")
                                continue
                            elif points_to_redeem > max_discount_points:
                                print(f"You can redeem maximum {max_discount_points} points!")
                                continue
                            elif points_to_redeem > current_points:
                                print(f"You dont have enough loyalty points")
                                continue
                            else:
                                break
                        except ValueError:
            ...
            discount = (points_to_redeem // 100)*10
            final_amount = max(0,subtotal - discount)
            points_to_earn = int(final_amount/10)
            print(f"\nFinal Booking Summary: ")
            print(f"Subtotal: ${subtotal}")
            if discount>0:                    
                print(f"Loyalty Discount: -${discount} ({points_to_redeem} points)")
            ...
            if confirm.lower() == 'back':
                ...
                seat_strings = selected_seats
                # Enhanced debug output
                print("\n=== DEBUG INFO ===")
                print(f"User ID: {self.current_user['id']} (type: {type(self.current_user['id'])})")
                print(f"Movie ID: {movie['id']} (type: {type(movie['id'])})")
                print(f"Theatre ID: {theatre['id']} (type: {type(theatre['id'])})")
                print(f"Show time: '{show_time}' (type: {type(show_time)})")
                print(f"Seat strings: {seat_strings} (type: {type(seat_strings)})")
                print(f"Points to redeem: {points_to_redeem} (type: {type(points_to_redeem)})")
                print("=================\n")
                if seat_strings:
                    print(f"First seat string: '{seat_strings[0]}' (type: {type(seat_strings[0])})")
                print("=================\n")
                try:
                    user_id = int(self.current_user['id'])
                    movie_id = int(movie['id'])
                    theatre_id = int(theatre['id'])
                    show_time_str = str(show_time)
                    points_to_redeem_int = int(points_to_redeem)
                    print(f"Calling with parameters:")
                    print(f"  user_id: {user_id} ({type(user_id)})")
                    print(f"  movie_id: {movie_id} ({type(movie_id)})")
                    print(f"  theatre_id: {theatre_id} ({type(theatre_id)})")
                    print(f"  show_time: '{show_time_str}' ({type(show_time_str)})")
                    print(f"  selected_seats: {selected_seats} ({type(selected_seats)})")
                    print(f"  points_to_redeem: {points_to_redeem_int} ({type(points_to_redeem_int)})")
                    
                    success, result = self.user.book_specific_seats_with_discount(
                        user_id, movie_id, theatre_id, show_time_str, selected_seats, points_to_redeem_int
                    )
                    if success:
                        print("Tickets booked successfully!")
                        if discount > 0:
                            print(f"You saved ${result['discount']} using loyalty points!")
                        print(f"You earned {result['points_earned']} new loyalty points!")
                        remaining_points = current_points - points_to_redeem + result['points_earned']
                        print(f"Your new loyalty points balance: {remaining_points}")
        ...
        # Right after the seat selection while loop ends, add this:
            print(f"FINAL DEBUG: selected_seats after selection = {selected_seats}")
            print(f"FINAL DEBUG: type of first seat = {type(selected_seats[0]) if selected_seats else 'No seats'}")                    
            if selected_seats:
                print(f"FINAL DEBUG: first seat content = {selected_seats[0]}")              
